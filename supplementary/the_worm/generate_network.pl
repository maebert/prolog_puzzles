...
set_size(X,Y) :- 
    retractall(size(_,_)), 
    assert(size(X,Y)),         % set new size
    MaxD is sqrt(X^2 + Y^2),   % calculate max distance
    retractall(constL(_)),    
...
connect_node([Name1, X1, Y1, Sec1, C1], [[Name2, X2, Y2, Sec2, C2] | OtherNodes]) :-
    connect([Name1, X1, Y1, Sec1, C1], [Name2, X2, Y2, Sec2, C2]), !,
    member(Name1, C2), member(Name2,C1),
    connect_node([Name1, X1, Y1, Sec1, C1], OtherNodes).

connect_node([Name1, X1, Y1, Sec1, C1], [_ | OtherNodes]) :-
    connect_node([Name1, X1, Y1, Sec1, C1], OtherNodes).
...
remove_nodes([[_,_,_,_, C] | R], Rest) :-
    length(C,0),!,
    remove_nodes(R, Rest).


% Removes isolated nodes
remove_nodes([[Name, X, Y, Sec, C] | R], [[Name, X, Y, Sec, C2] | Rest]) :-
    list_to_set(C,C2),
    remove_nodes(R, Rest).
...
names2matlab([]).
names2matlab([[N,X,Y,S,_] | T]) :-
    sec_levels(SMax), S2 is 1 - (S / SMax),
    write('text('),write(X),write(','),write(Y),write(','),
    write('\''),write(N), write('\','), 
...
row2matlab(_, []):- write(' ; ').
row2matlab([Name,X,Y,Sec,C], [[Name2,_,_,_,_] | T]) :-
    (member(Name2,C) -> write(1) ; write(0)),
    write(' '),
    row2matlab([Name,X,Y,Sec,C], T).
   
% III.b ) For Prolog
   
write_database([]).    
write_database([[Name,_,_,Sec,C] | R]) :-
